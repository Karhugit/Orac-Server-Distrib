...
from resources.scrapers.modules.control import existsPath, dataPath, makeFile, cacheFile
...
def get(function, duration, *args):
	...
	try:
		...
		log(f"Cache error in {_get_function_name(function)}", LOGERROR)
		return None
...
def cache_get(key):
	try:
		...
		log(f"Cache get error for key: {key}", LOGERROR)
		return None
	finally:
...
def cache_insert(key, value):
	try:
		...
		log(f"Cache insert error for key: {key}", LOGERROR)
	finally:
		dbcur.close() ; dbcon.close()
