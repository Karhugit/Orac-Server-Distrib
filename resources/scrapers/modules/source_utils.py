...
from resources.scrapers.modules import cleantitle
from resources.scrapers.modules.undesirables import Undesirables
from resources.scrapers.modules.control import homeWindow, setting as getSetting, setSetting
...
def get_undesirables():
	if getSetting('filter.undesirables') == 'false' or home_getProperty('fs_filterless_search') == 'true' : return []
...
def get_release_quality(release_info, release_link=None):
	try:
		...
		log_utils.log("Error in get_release_quality", level=log_utils.LOGERROR)
		return 'SD', []
...
def aliases_to_array(aliases, filter=None):
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in aliases_to_array", level=log_utils.LOGERROR)
		return []
	...
	if aliases:
		for item in aliases:
			try:
				...
				from resources.lib import log_utils
				log_utils.log("Error in check_title loop", level=log_utils.LOGDEBUG)
	try:
...
# filter to remove episode ranges that should be picked up in "filter_season_pack()" ex. "s01e01-08"
		...
		from resources.lib import log_utils
		log_utils.log("Error in check_title", level=log_utils.LOGERROR)
		return False
...
def remove_lang(release_info, check_foreign_audio):
	...
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in remove_lang", level=log_utils.LOGERROR)
		return False
	...
	if aliases:
		for item in aliases:
			try:
				...
				from resources.lib import log_utils
				log_utils.log("Error in filter_season_pack aliases loop", level=log_utils.LOGDEBUG)
	try:
...
# remove season ranges - returned in showPack scrape, plus non conforming season and specific crap
		...
		if any(i in rt for i in string_list):
			for item in (
				...
				season_full_fill_check.rstrip('.') + r'[.-]season[.-]\d{2}(?:[.-]|$)' # 2 digit season followed by 2 digit season range x."season.01-season.09."
					):
				if bool(re.search(item, release_title)): return False, 0, 0
			return True, 0, 0
		return False, 0, 0
		...
		from resources.lib import log_utils
		log_utils.log("Error in filter_season_pack", level=log_utils.LOGERROR)
		return True
	...
	if aliases:
		for item in aliases:
			try:
				...
				from resources.lib import log_utils
				log_utils.log("Error in filter_show_pack aliases loop", level=log_utils.LOGDEBUG)
	try:
		# show_title = show_title.replace('!', '').replace('(', '').replace(')', '').replace('&', 'and')
		show_title = show_title.replace('!', '').replace('(', '').replace(')', '').replace('&', 'and').replace(year, '') # year only in meta title if an addon custom query added it
		if show_title not in title_list: title_list_append(show_title)
...
# "s01.s09" 2 digit  range filtering (dots)
		...
		from resources.lib import log_utils
		log_utils.log("Error in filter_show_pack", level=log_utils.LOGERROR)
		return True, total_seasons
...
def info_from_name(release_title, title, year, hdlr=None, episode_title=None, season=None, pack=None):
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in info_from_name", level=log_utils.LOGERROR)
...
def strip_non_ascii_and_unprintable(text):
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in strip_non_ascii_and_unprintable", level=log_utils.LOGERROR)
		return text
...
def _size(siz):
	try:
		...
		from resources.lib import log_utils
		log_utils.log('failed on siz=%s' % siz, level=log_utils.LOGERROR)
		return 0, ''
...
def convert_size(size_bytes, to='GB'):
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in convert_size", level=log_utils.LOGERROR)
		return 0, ''
...
def base32_to_hex(hash, caller):
	...
	log_utils.log('%s: base32 hash  "%s"  converted to hex 40  "%s" ' % (caller, hash, hex), level=log_utils.LOGDEBUG)
	return hex
...
def is_host_valid(url, domains):
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in is_host_valid", level=log_utils.LOGERROR)
		return False, ''
...
def __top_domain(url):
	...
	try:
		...
		from resources.lib import log_utils
		log_utils.log("Error in __top_domain", level=log_utils.LOGERROR)
...
def copy2clip(txt):
	...
	if platform == "win32":
		try:
			...
			from resources.lib import log_utils
			log_utils.log('Windows: Failure to copy to clipboard', level=log_utils.LOGERROR)
		...
		try:
			...
			from resources.lib import log_utils
			log_utils.log('Mac: Failure to copy to clipboard', level=log_utils.LOGERROR)
	elif platform == "linux":
		try:
			...
			from resources.lib import log_utils
			log_utils.log('Linux: Failure to copy to clipboard', level=log_utils.LOGERROR)