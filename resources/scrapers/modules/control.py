...
def setting(id, fallback=None):
	try: settings_dict = jsloads(homeWindow.getProperty('orac_settings'))
	except: settings_dict = {}
	if settings_dict is None: settings_dict = {}
...
def make_settings_dict(): # service runs upon a setting change
	try:
		...
		homeWindow.setProperty('orac_settings', jsdumps(settings_dict))
		return settings_dict
	except:
		return None

def refresh_debugReversed(): # called from service "onSettingsChanged" to clear orac.log if setting to reverse has been changed
	if homeWindow.getProperty('orac.debug.reversed') != setting('debug.reversed'):
		homeWindow.setProperty('orac.debug.reversed', setting('debug.reversed'))
		execute('RunPlugin(plugin://script.module.orac/?action=tools_clearLogFile)')
...
def isVersionUpdate():
	...
	try:
		...
		xbmc.log('Orac Addon Data Path Does not Exist. Creating Folder....', LOGINFO)
		addon_folder = transPath('special://profile/addon_data/script.module.orac')
	...
	try:
		...
		log('Error occurred while handling version check', LOGWARNING)
		return False
...
def clean_settings():
	...
	try:
		...
		log('Error occurred while cleaning settings', LOGWARNING)
