...
from resources.scrapers.modules import dom_parser
	...
	try:
		...
		if not verifySsl and version_info >= (2, 7, 12):
			try:
				...
				log(f'Error disabling SSL verification for {url}', LOGWARNING)
			...
			if not ignore:
				if response.code in (301, 307, 308, 503, 403): # 403:Forbidden added 3/3/21 for cloudflare, fails on bad User-Agent
					...
					if flare and 'cloudflare' in str(response.info()).lower():
						log('client module calling cfscrape: url=%s' % url, level=LOGDEBUG)
						try:
							from resources.scrapers.modules import cfscrape
							...
							except: log('Error getting cookies from cfscrape request', LOGWARNING)
							if response.status_code == 403: # if cfscrape server still responds with 403
								log('cfscrape-Error url=(%s): %s' % (url, 'HTTP Error 403: Forbidden'), LOGDEBUG)
								log('Headers-Used: %s' % str(headers), LOGDEBUG)
								return None
						except:
							log('Error occurred while handling cfscrape response', LOGWARNING)
						...
						if error is False:
							log('Request-Error url=(%s)' % url, LOGDEBUG)
							return None
				else:
					if error is False:
						log('Request-Error url=(%s)' % url, LOGDEBUG)
		...
		log('Request-Error url=(%s)' % url, LOGWARNING)
		return None
...
def _add_request_header(_request, headers):
	try:
		...
		log('Error occurred while adding request headers', LOGWARNING)
...
class cfcookie:
	...
	def parseJSString(self, s):
		try:
			...
			log('Error occurred while parsing JS string', LOGWARNING)
