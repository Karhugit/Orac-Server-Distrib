...
from .modules.undesirables import Undesirables
from .modules.control import homeWindow, setting as getSetting, setSetting
...
def get_release_quality(release_info, release_link=None):
	try:
		...
		log('Error occurred while determining release quality', LOGWARNING)
		return 'SD', []

def get_pack_urls(base_link, search_queries, scraper_name, link_regex_pattern):
	"""
	Fetches and parses multiple search pages to get a list of detail page URLs for packs.
	
	:param base_link: The base URL of the site.
	:param search_queries: A list of URL paths for the search queries.
	:param scraper_name: The name of the scraper for logging.
	:param link_regex_pattern: The regex pattern to find detail links in the search page HTML.
	:return: A list of unique, absolute URLs to detail pages.
	"""
	all_item_links = []
	...
	for query in search_queries:
		try:
			...
			links = re.findall(link_regex_pattern, html)
			for link in links:
				absolute_link = urljoin(base_link, link)
				all_item_links.append(absolute_link)
		except Exception:
			scraper_error(scraper_name)
			
	return list(set(all_item_links))
...
def aliases_to_array(aliases, filter=None):
	try:
		...
		log('Error occurred while processing aliases', LOGWARNING)
		return []
	...
	if aliases:
		for item in aliases:
			try:
				...
				log('Error occurred while processing aliases', LOGWARNING)
	try:
		# Add the main title to the list of possible titles to check
		main_title = title.replace('&', 'and')
		if main_title not in title_list:
			title_list.append(main_title)

		# Clean the release title for comparison
		cleaned_release_title = cleantitle.get(release_title)

		# Check if any of the possible titles match the release title
		match_found = False
		for t in title_list:
			ct = cleantitle.get(t)
			if ct in cleaned_release_title:
				# If the title is short or a single word, substring matching is prone to false positives.
				# Verify with a stricter word boundary check on the title portion of the release.
				if len(ct) <= 5 or ' ' not in t:
					title_part = release_title
					if years: # For movies, split by year
						for y in years:
							match = re.search(r'(?<!\d)%s(?!\d)' % y, title_part)
							if match:
								title_part = title_part[:match.start()]
								break
					else: # For shows, split by handler
						match = re.search(r'%s' % hdlr, title_part, re.I)
						if match:
							title_part = title_part[:match.start()]
					
					title_part = title_part.replace(str(year), '')
					dot_title_part = release_title_format(title_part)
					dot_t = release_title_format(t)
					
					if dot_title_part == dot_t or dot_title_part.endswith(dot_t) or dot_title_part.startswith(dot_t):
						match_found = True
						break
				else:
					match_found = True
...
# filter to remove episode ranges that should be picked up in "filter_season_pack()" ex. "s01e01-08"
		...
		log(f'Error in check_title with title="{title}", release_title="{release_title}", hdlr="{hdlr}", year="{year}"', LOGWARNING)
		return False
...
def remove_lang(release_info, check_foreign_audio):
	...
	try:
		...
		log('Error occurred while checking language', LOGWARNING)
		return False
...
# remove season ranges - returned in showPack scrape, plus non conforming season and specific crap
		...
		log('Error occurred while filtering season pack', LOGWARNING)
		return True
...
# "s01.s09" 2 digit  range filtering (dots)
		...
		log('Error occurred while filtering show pack', LOGWARNING)
		# return True, total_seasons
...
def info_from_name(release_title, title, year, hdlr=None, episode_title=None, season=None, pack=None):
	try:
		...
		log('Error occurred while extracting info from name', LOGWARNING)
...
def release_title_format(release_title):
	try:
		...
		log('Error occurred while formatting release title', LOGWARNING)
		return release_title
...
def clean_name(release_title):
	try:
		...
		log('Error occurred while cleaning name', LOGWARNING)
		return release_title
...
def strip_non_ascii_and_unprintable(text):
	try:
		...
		log('Error occurred while stripping non-ascii characters', LOGWARNING)
		return text
...
def _size(siz):
	try:
		...
		log('failed on siz=%s' % siz, LOGWARNING)
		return 0, ''
...
def convert_size(size_bytes, to='GB'):
	try:
		...
		log('Error occurred while converting size', LOGWARNING)
		return 0, ''

def base32_to_hex(hash, caller):
	from base64 import b32decode
	hex = b32decode(hash).hex()
	log('%s: base32 hash  "%s"  converted to hex 40  "%s" ' % (caller, hash, hex), __name__, LOGDEBUG)
	return hex

def scraper_error(provider):
	import traceback
	failure = traceback.format_exc()
	log(provider.upper() + ' - Exception: \n' + str(failure), level=LOGERROR)
...
def is_host_valid(url, domains):
	try:
		...
		log('Error occurred while validating host', LOGWARNING)
		return False, ''
...
def __top_domain(url):
	...
	try:
		...
		log('Error occurred while extracting top domain', LOGWARNING)
...
def copy2clip(txt):
	...
	if platform == "win32":
		try:
			...
			log('Windows: Failure to copy to clipboard', LOGWARNING)
		...
		try:
			...
			log('Mac: Failure to copy to clipboard', LOGWARNING)
	elif platform == "linux":
		try:
			...
			log('Linux: Failure to copy to clipboard', LOGWARNING)